...
(function () {
  ...
  SensorApp.registerIntegration({
    ...
    async call(method, params, creds) {
      ...
      var json = await request(url, creds, body, this.dailyFreeLimit);
  ...
  /* ════════════════════════════════════════════════════════════════════════
     Низкоуровневый запрос: заголовки, разбор тела, дружелюбные коды ошибок.
     ════════════════════════════════════════════════════════════════════════ */
  async function request(url, creds, body, dailyLimit) {
    ...
    if (status === 402) {
      throw new Error('Закончился баланс/лимит DaData (HTTP 402). Пополните тариф ' +
        'или дождитесь сброса суточного лимита (' + (dailyLimit || 10000) + ' запросов/сутки на бесплатном).');
    }
    if (status === 429) {
      ...
      throw new Error('Превышен лимит запросов DaData (HTTP 429' +
        ...
        'Бесплатный тариф — ' + (dailyLimit || 10000) + ' запросов/сутки и ограничение по частоте. ' +
